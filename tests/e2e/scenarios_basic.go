	...
	return &harness.Scenario{
		...
		Steps: []harness.Step{
			...
			harness.NewStep("help command", func(ctx *harness.Context) error {
				...
				if !strings.Contains(result.Stdout, "Tools for using Anthropic/Claude API plaform") {
					return fmt.Errorf("expected help output to contain 'Tools for using Anthropic/Claude API plaform'")
				}
				return nil
			}),
		},
